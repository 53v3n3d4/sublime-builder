 ...
 */
function writeStFile (jsonFile, title, author, licenseName, filename, keyname, tabSpaces) {
  try {
    const tabs = typeof tabSpaces === 'number' ? tabSpaces : 4
...
const messageErrorTab = 'tabSpaces should be a number, max number = 10'
 ...
 * @param {number} tabSpaces - Indentation used on build file, tab spaces. Max number = 10.
 * replaced.
 * @example theme,
 * builder(
 *   'merge',
...
export function builder (
  ...
  } else if (typeof tabSpaces === 'string' || tabSpaces > 10 || typeof tabSpaces === 'boolean' ||
    tabSpaces instanceof Array) {
