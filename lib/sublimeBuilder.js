...
/**
 * Performs a deep merge of objects and returns new object. Does not
 * modify objects (immutable) and merges arrays via concatenation.
...
/**
 * Replace variables keys with values in rules themes or globals/rules
 * color-schemes.
 * Takes around 1 min for theme building. Appear that with for loops
 * was faster.
 * @param {Object} sublime text rules.
 * @param {Array.<Object>} sublime text variables - E.g. colors-pallete
 *        and adaptive-vars files.
 * @returns {Object} sublime file rules - Variables replaced.
 */
function replaceVars (obj, arr) {
  const total = Count2.total(obj)
  // Progress bar smoother when use numbers instead of Counter.total method.
  // Same time for building,but with Counter.total method, bar is slow until
  // 25% then finishes instantaneous.
  ...
  const result = Object.entries(obj)
    ...
    .map(([k, v]) => {
      if (v == null) {
        ...
        // Running twice because vars reused inside same file as values.
        // E.g. var(vcs-deleted)
          ...
          arr.map(obj2 =>
            Object.entries(obj2.variables).map(([k2, v2]) => {
              Object.entries(k2).forEach(v3 => {
                ...
                obj[k] = typeof v2 !== 'object'
                  ? obj[k].toString().replaceAll(key3, v2)
                  : obj[k]
                // Probably better option try check if scheme or theme
                // instead of keys.
                // Failed to check parent key globals presence.
                // Theme files using {number}, but color-scheme files
                // use {string} numbers.
                obj[k] = isFinite(obj[k]) && !arrayInclude(globalsKeys, k)
                  ? +obj[k]
                  : obj[k]
                obj[k] = typeof v2 === 'object' && v === key3
                  ? obj[k] = value3
                  : obj[k]
                // Return empty vars that change to 0.
                // E.g. "var(selection-fg)" = ""
                obj[k] = obj[k] === 0 && typeof v === 'string'
                  ? obj[k] = ''
                  : obj[k]
              })
              return { [k]: obj[k] }
...
// Need if using ReplaceVars, replace for stringfy objects values on st
// variables,
// E.g. "{\"target\":1,\"speed\":4,\"interpolation\":\"smoothstep\"}"
export const replaceData2 = (data) => data.replace(/\\"/g, '"')
  .replace(/("\{|\}")/g, m => m === '"{' ? '{' : '}')
...
function writeStFile (
  ...
  try {
    const tabs = typeof tabSpace === 'number' ? tabSpace : 4
    const keyName = { name: capitalize(keyname) }
    const updateFile = keyname == null
      ? jsonFile
      : addElements(jsonFile, keyName)
    const str = replaceData2(JSON.stringify((updateFile), null, tabs))
    const copyright = license(title, author, licenseName)
    const themeLicense = copyright + str
    fs.writeFileSync(filename, themeLicense)
...
const messageErrorTab = 'tabSpace should be a number, max number = 10'

/**
 * Builder: read json, merge files or replace variables, write sublime
 * text file.
 ...
 * @param {string} title - Title is used on top of build file together
 *        with license and author.
 *        E.g. 'Treble Adaptive.sublime-theme - Treble Adaptive'
 * @param {string} author - Author name appear at top of build file.
 * @param {string} licenseName - Software license agreement at top of
 *        build file.
 * @param {string} filename - Filename with extension.
 *        E.g. Treble Adaptive.sublime-theme
 * @param {string} keyname - Color-schemes has a key 'name' at top of file.
 *        E.g. "name": "Blackcomb"
 * @param {Object} rulesFile - Replace option, all rules and globals will
 *        have variables values
 * @param {number} tabSpace - Indentation used on build file, tab spaces.
 *        Max number = 10.
 * @example theme:
 *          builder(
 *            'merge',
...
export function builder (
  ...
  if (!['merge', 'replace'].includes(kind)) {
    ...
    typeof tabSpace === 'string' ||
    tabSpace > 10 ||
    typeof tabSpace === 'boolean' ||
    tabSpace instanceof Array
  ) {
    console.log(color(messageErrorTab, 'red'))
    return messageErrorTab
  } else {
    const filesArray = stFiles.map(readAndConvertJson)
    if (kind === 'merge') {
      const mergeFile = mergeDeep(...filesArray)
      const buildFile = writeStFile(
        mergeFile,
        title,
        author,
        licenseName,
      ...
      // Reset counters for Watch. Otherwise, counters accumulate on each run
      // and mess progress bar.
      Count.reset()
      Count2.reset()
      return buildFile
    }
