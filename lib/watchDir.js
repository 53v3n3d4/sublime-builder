 ...
 * @param {Array.<Object>} stFilenames - Array of objects with keys, name
 *        and filenames, for each theme/color-scheme.
 ...
 * @example
 * watchDir(
 *   themeWatch.folderPath, themeWatch.folder, themeWatch.themes, sublimeFile
 * )
 */
export function watchDir (stFolder, folderName, stFilenames, fn) {
  ...
  watcher
    .on('add', path => {
      console.log('File ' + color(path, 'cyan') + ' has been added')
    })

    // Rebuild sublime files when source files change.
    // Option awaitWriteFinish enabled: if file is big, saving file that are
    // being build will rebuild after finish.
    .on('change', (path) => {
